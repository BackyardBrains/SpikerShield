/*
  * ----------------------------------------------------------------------------------------------------
  * Backyard Brains 16. Aug. 2018
  * 
  * Muscle SpikerShield with made for Max's claw
...
//deg: -100 to +100
#define MIN_ANGLE_DEG 15.0
#define MAX_ANGLE_DEG 100.5

#define NOISE_LEVEL_OPEN 40
#define NOISE_LEVEL_CLOSE 120

float autoShutDownAfterMinutes = 0.5;
...
uint16_t secondsForShutDownTimer = 0;
uint16_t secondsForShutDownTimerMax = 0;

bool servoEnabled = true;
uint16_t secondCounter = 0;
int minimumPeriods = 0;
int maximumPeriods = 0;
//----------------------------------- SETUP FUNCTION --------------------------------------------------------------------------------------------
void setup()
{

    float minNumOfPeriods = (MIN_ANGLE_DEG/0.107+1500)/50.0;
    float maxNumOfPeriods = (MAX_ANGLE_DEG/0.107+1500)/50.0;
    float numberOfPeriodsSpan = (maxNumOfPeriods - minNumOfPeriods);
    //(sens-40)/numberOfPeriodsSpan
    int i;
    for (i=0;i<6;i++)
    {
        incrementsForOpenClaw[i] = (int)((sensitivities[i]-NOISE_LEVEL_OPEN)/numberOfPeriodsSpan);
        incrementsForClosedClaw[i] = (int)((sensitivities[i]-NOISE_LEVEL_CLOSE)/numberOfPeriodsSpan);
    }
    
    minimumPeriods = (int)minNumOfPeriods;
    maximumPeriods = (int) maxNumOfPeriods;

    secondsForShutDownTimerMax  = (int)(autoShutDownAfterMinutes*60.0);
  ...
  //do the auxiliary computation once per timer interrupt
   if(readyToDoAuxComputation==1)
   {
        ...
        if(sensitivityVisualFeedbackCounter==0)//disable update of LEDs when we display selected sensitivity level
        {
                  ...
                  if(envelopeFirstChanel>movingThresholdSum)
                  {
                      PORTD |= B01000000;//PORTB |= B00001000;//turn ON LED ----- PB3
                     //PORTD &= B10111111;
                     secondsForShutDownTimer = 0;
                          servoEnabled = true;
                  }
                  else
                  {
                      
                      PORTD &= B10111111; //PORTB &= B11110111;//turn OFF LED
                       secondCounter++;
                          if(secondCounter==10000)
                          {
                            secondCounter = 0;
                          
                                if(servoEnabled)
                                {
                                  secondsForShutDownTimer++;
      
                                  if(secondsForShutDownTimer == secondsForShutDownTimerMax)
                                  {
                                    secondsForShutDownTimer = 0;
                                    servoEnabled = false;
                                  }
        ...
        if(disableServoMeasure==0)
        {
              ...
              if(currentFunctionality == OPEN_CLAW_MODE)
              {
                ...
                tempActiveServoPeriod = maximumPeriods;//PWM ON period = 48*50us = 2400us
                for(movingThresholdSum = NOISE_LEVEL_OPEN;movingThresholdSum <emgSaturationValue;movingThresholdSum+=incrementForOpenClawThreshold)
                {
              ...
              else
              {
                ...
                tempActiveServoPeriod = minimumPeriods;//PWM ON period = 32*50us = 1600us
                for(movingThresholdSum = NOISE_LEVEL_CLOSE;movingThresholdSum <emgSaturationValue;movingThresholdSum+=incrementForClosedClawThreshold)
                {
                  if(  movingThresholdSum>envelopeFirstChanel)
                  {
